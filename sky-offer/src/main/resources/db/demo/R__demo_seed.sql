...
VALUES (1, 'Sopot Beach Hotel', 'Charming beachside hotel in Sopot with direct access to the Baltic Sea.',
       ...
       (2, 'Warsaw City Suites', 'Modern suites in the heart of Warsaw, walking distance from the Old Town.',
       ...
       (3, 'Miami Ocean Resort', 'Luxury resort on Miami Beach with ocean views and pool access.',
...
SELECT setval(pg_get_serial_sequence('offer', 'id'), COALESCE((SELECT MAX(id) FROM offer), 1),
              (SELECT COUNT(*) FROM offer) > 0);

INSERT INTO offer_event (id, offer_id, sequence_number, event_type, payload, timestamp)
VALUES (1, 1, 1, 'OFFER_CREATED',
        '{"id":1,"hotelName":"Sopot Beach Hotel","ownerEmail":"owner@sky.dev","city":"Sopot","country":"Poland","price":320.00}',
        now()),
       (2, 2, 1, 'OFFER_CREATED',
        '{"id":2,"hotelName":"Warsaw City Suites","ownerEmail":"owner@sky.dev","city":"Warsaw","country":"Poland","price":480.00}',
        now()),
       (3, 3, 1, 'OFFER_CREATED',
        '{"id":3,"hotelName":"Miami Ocean Resort","ownerEmail":"owner@sky.dev","city":"Miami","country":"USA","price":950.00}',
        now())
ON CONFLICT
    (id)
    DO NOTHING;

SELECT setval(pg_get_serial_sequence('offer_event', 'id'), COALESCE((SELECT MAX(id) FROM offer_event), 1),
              (SELECT COUNT(*) FROM offer_event) > 0);
