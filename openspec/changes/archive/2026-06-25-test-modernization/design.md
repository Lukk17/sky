...
- E2E happens automatically every PR via Newman.
...
7. **Newman runs against docker-compose**, not Helm in CI. Helm-against-kind takes 5+ minutes; compose is sub-minute and exercises the same images.
...
- **Newman flakiness**: E2E against a live stack can flake on slow CI runners. Mitigation: explicit waits for `/actuator/health`, retry on connection refused.
