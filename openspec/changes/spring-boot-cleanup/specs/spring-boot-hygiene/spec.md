...
No `application.yml` or related profile config MUST carry a default value for a credential, secret, or password env-var reference. Missing env vars MUST cause startup to fail loudly.
...
The default CORS allowed-origins value in `application.yml` MUST be a named known origin (the production frontend), not a wildcard. Dev profile overlays may relax to localhost origins; wildcards are forbidden in defaults.
