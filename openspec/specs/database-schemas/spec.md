...
## Purpose
Fixes where a service's tables and its own migration history live, so one service's schema evolution cannot collide with another's. It states the ownership boundary rather than the engine, which has already changed once.
