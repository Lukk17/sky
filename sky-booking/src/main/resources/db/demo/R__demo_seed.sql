...
VALUES (1, '1', '2027-07-15', 'user@sky.dev', 'owner@sky.dev'),
...
VALUES (1, 1, 1, 'BOOKED',
        '{"bookingId":1,"offerId":"1","bookingUser":"user@sky.dev","ownerEmail":"owner@sky.dev","bookedDate":"2027-07-15"}',
        now()),
       (2, 2, 1, 'BOOKED',
        '{"bookingId":2,"offerId":"2","bookingUser":"user@sky.dev","ownerEmail":"owner@sky.dev","bookedDate":"2027-08-20"}',
        now())
ON CONFLICT
    (id)
    DO NOTHING;

SELECT setval(pg_get_serial_sequence('booking_event', 'id'), COALESCE((SELECT MAX(id) FROM booking_event), 1),
              (SELECT COUNT(*) FROM booking_event) > 0);
