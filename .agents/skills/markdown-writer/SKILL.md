...
- Skill manifests (`SKILL.md`): agent-facing, so the human-voice and AI-tell prose rules are relaxed. The
  structural rules now apply and are enforced by `tools/check-markdown.py`: no em-dashes or en-dashes, sections at
  `###` with a `---` divider before each, and prose wrapped at 120. Subagent definitions (`.claude/agents/*.md`,
  `.opencode/agents/*.md`, `subagents/*.md`) keep their own conventions and are generated, so edit `subagents/*.md`
  and regenerate rather than hand-editing the generated copies.
...
- Never use em-dashes or en-dashes. These are the single most reliable AI tell in 2026: they appear
  ...
  `agent-standards`, `read-only`, `opinionated-template` are fine; the em-dash (U+2014) and en-dash (U+2013)
  characters themselves are banned. Run `grep -P "[\x{2013}\x{2014}]" file.md` after editing to verify.
- Don't write in ways that scream "AI wrote this". Per 2026 detection research, the strongest tells are below.
  ...
  - Three-part rhythm chains. The em-dash form is already banned by the dash rule, but the rhythm reads as AI even
    with other punctuation ("fast, reliable, affordable; tested, scalable, secure"). Vary or split.
...
- Section headings in human-facing markdown start at `###` (H3), not `#` or `##`. Reserve `#` (H1) for the
  ...
  are `####`, sub-sub-sections are `#####`. This now applies to skill manifests (`SKILL.md`) too, enforced by
  `tools/check-markdown.py`. Subagent files and the `AGENTS.md` / `CLAUDE.md` families keep their own conventions.
- Every section gets a `---` horizontal-rule divider before its heading, including the first section after the
  ...
  (`###` per the rule above, in both READMEs and skill manifests); the rule is the same regardless of the level
...
- ❌ Em-dashes (U+2014) or en-dashes (U+2013) anywhere in body text or headings: replace with comma, period, colon, or
