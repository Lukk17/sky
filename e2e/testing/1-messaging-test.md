...
The run summary reports Status PASS with all requests passed and all assertions passed: 18/18 assertions.
...
Postgres `public.message`; and the cleanup delete returns HTTP 204.
...
- Mutates: Postgres `public.message`, rows where `sender_email = lukk@sky.dev` (the canary messages this test creates
  and deletes).
- Conflicts with: none. No other test in the suite writes to `public.message`.
- Serial: false.
