...
- **Stack**: Java 21, Spring Boot 3.5.4, Spring Web + WebFlux, Spring Data JPA, Spring Kafka, Flyway, springdoc-openapi 2.6 (Swagger UI at `/swagger-ui/index.html`), Lombok 1.18.38, JUnit 5.11, Gradle Kotlin DSL.
...
- **Build**: single composite Gradle build — one root `gradlew`, modules wired in `settings.gradle.kts`, shared config in `buildSrc/` convention plugins, versions in `gradle/libs.versions.toml`. The JDK 25 toolchain is auto-resolved via the Foojay plugin and ready for the Spring Boot 4 migration, but the catalogue is currently pinned to Java 21 / Spring Boot 3.5.4.
