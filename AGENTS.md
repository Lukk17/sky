...
- API: REST under a `/api/v1` URL prefix (the services' own `sky.apiPrefix`), and the edge neither strips it nor adds anything in front of it. The published path is the path the service serves: `/api/v1/offers` at `http://localhost:5777` and at `https://skycloud.luksarna.com` is `/api/v1/offers` on sky-offer. The service-naming prefixes `/offer/api`, `/booking/api` and `/msg/api` and the rewriting behind them are gone from the gateway and from every ingress. What makes that possible is that the three REST services own disjoint top-level resources, `offers`, `search` and `owner/offers` on sky-offer, `bookings` and `user/bookings` on sky-booking, `messages` on sky-message, so the edge matches on the resource and forwards unchanged. A new top-level resource therefore has to stay disjoint from the other two services and has to be added to the gateway route and to the service's ingress, or it is unreachable from outside. Swagger UI and the api-docs are the exception and keep their service prefix and their rewrite, because all three services serve them on the same path. Earlier `hello` probe endpoints were dropped. Every error answers `application/problem+json`, with no second shape anywhere: `UnhandledExceptionResolver` in `sky-common` catches whatever no advice mapped and answers a 500 problem detail that names no exception, no message and no class, so Spring Boot's flat `{timestamp, status, error, path}` body no longer reaches a client. It is a `HandlerExceptionResolver` at `Ordered.LOWEST_PRECEDENCE` rather than a `@ControllerAdvice`, because an advice cannot sort behind the service advices and a catch-all that wins would turn every 503, 502, 409 and 404 into a 500. See [sky-common/AGENTS.md](sky-common/AGENTS.md) before touching it.
